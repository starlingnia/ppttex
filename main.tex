...
% ==============================================================================
\begin{frame}
\myheading{Lattice QCD Numerical Setup at physical point}
\vfill
...
\begin{itemize}
    ...
    \item The total number of configurations ($N_{\mathrm{conf}}$) at physical point analyzed for each temperature/$\beta$ are counted below :
\end{itemize}
...
\begin{itemize}
    \item \textbf{Local Effective Mass}: Numerically solve the ratio at each spatial coordinate $x$ to obtain the local effective mass $m_{\text{eff}}(x)$:
    \[
        \frac{C(x)}{C(x + 1)} = \frac{\cosh(m_{\text{eff}}(x)(x - L/2))}{\cosh(m_{\text{eff}}(x)(x + 1 - L/2))}
    \]

    \item \textbf{Plateau and Range}: The plateau of $m_{\text{eff}}(x)$ at large separations is used to determine a suitable fitting range $[x_{\text{min}}, x_{\text{max}}]$.

    \item \textbf{Screening Mass Fit}: The physical screening mass $m_{\text{scr}}$ is extracted by performing a correlated $\chi^2$ fit of $C(x)$ to the asymptotic form:
    \[
        C(x) = A \cosh\left(m_{\text{scr}}\left(x - \frac{L}{2}\right)\right)
    \]

    \item \textbf{Statistical Error}: Jackknife method and Binning analysis are used to handle statistical uncertainties and eliminate HMC autocorrelation bias.
\end{itemize}
\vfill
\end{frame}
...
% ==============================================================================
% Slide 14b: Chiral Condensate (手征凝聚)
% ==============================================================================
\begin{frame}
\myheading{Chiral Condensate \& Crossover Phase Transition}
\vfill
\begin{columns}[c]
    \begin{column}{0.55\textwidth}
        \centering
        \includegraphics[width=0.49\textwidth]{conden_plot.png}
        \includegraphics[width=0.49\textwidth]{conden_re_plot.png} \\
        \vspace{0.1cm}
        {\tiny Left: Bare Chiral Condensate, Right: Renormalized Chiral Condensate}
    \end{column}
    \begin{column}{0.42\textwidth}
        ...
        {\small
        \[
            a^3 \langle \bar{\psi}\psi_q \rangle = \frac{1}{N_s^3 N_\tau} \frac{N_f}{4} \langle \mathrm{Tr} \slashed{D}_q^{-1} \rangle
        \]}
        \begin{itemize}
            \item Order parameter for spontaneous chiral symmetry breaking
            \item We subtract strange quark part to remove divergence
        \end{itemize}
    \end{column}
\end{columns}
\vfill
...
\begin{columns}[c]
    ...
    \begin{column}{0.45\textwidth}
        ...
        \begin{itemize}
            ...
            \item Clear peak determines pseudo-critical temperature: \textbf{$T_c \approx 158$ MeV}.
        \end{itemize}
    \end{column}
\end{columns}
\vfill
...
\vspace{0.5ex}
\begin{itemize}
    \item \textbf{Physical Point}: Crossover transition with $T_c \approx 158$ MeV. Above $T_c$, chiral and $U(1)_A$ symmetry breakings are consistent with zero within errors.
    \item \textbf{Chiral Limit Study}: Extrapolations to $am_l \to 0$ support simultaneous restoration of chiral flavor and $U(1)_A$ symmetries (with $T_c \approx 165$ MeV).
    \item \textbf{Chiral-Spin Symmetry}: Remains broken up to 220 MeV, indicating active non-perturbative chromomagnetic interactions in QGP.
    \item \textbf{Transition Scenario}:
        \begin{itemize}
            \item \textbf{Previous}: 1st-order if $U(1)_A$ restored; 2nd-order $O(4)$ if $U(1)_A$ remains broken.
            \item \textbf{New}: $O(4)$ if $U(1)_A$ remains; weak 1st-order, or 2nd-order ($O(4)\times Z_2$, $O(4)\times Z_4$, $O(4)\times O(2)$) if $U(1)_A$ restored.
        \end{itemize}
