...
\fontsize{11pt}{14pt}\selectfont
\begin{itemize}
\item  We extend this investigation to a more realistic $N_f=2+1$ QCD case adding strange quark and  improve it to finer lattice size up to L$48^3\times \rm T16(18)$ 
\item  We study the physical point case for 
...
\begin{itemize}
\item Perturbative analysis of screening masses from NRQCD$_3$ {\scriptsize{[Laine \& Vepsäläinen (2004)]}},%\[ \left[ 2 M - \frac{\nabla_{\ensuremath{\boldsymbol{r}}}^2}{\pi T} + V(\ensuremath{\boldsymbol{r}}) \right]\Psi_0 = m_{\ensuremath{\operatorname{scr}}} \Psi_0 \]

solving the equation for the $N_f=2$ case yields:
...
\begin{itemize}
    ...
    The massless operator $D_{\mathrm{ov}}(0) = \frac{1}{2} (1 + \gamma_5 \mathrm{sgn}(H_W))$ 
    with Hermitian $ H_W$, satisfies the Ginsparg-Wilson relation:
    ...
    DWF approximates the sign function $\mathrm{sgn}(H_W)$ by introducing a 5th dimension of size $L_s$.
     The effective 4D operator is:
    \[ D^{\text{4D}}_{\mathrm{DW}} (m) = \frac{1+ m}{2} + \frac{1 - m}{2} \gamma_5 \tanh (L_s \tanh^{- 1} (H_M)) \]
   It converges exponentially to the exact overlap operator $D_{\mathrm{ov}}(m)$.
\end{itemize}
\vfill
\end{frame}
...
{5D Möbius Domain Wall Dirac Operator ($L_s = 12$)}:
\vspace{-0.3em}
{\small
...
\ \ \ where {\small $D_{+}^{(s)} = b_5 D_W + 1$, $D_{-}^{(s)} = c_5 D_W - 1$, 
and $P_\pm = \frac{1 \pm \gamma_5}{2}$.} 

It corresponds to $H_{\rm MDWF} = \frac{\gamma_5(b+c) D_W}{2+(b-c)D_W}$, and we use $b=1.5,\ c=0.5$.

\vspace{0.4em}
{Residual Mass $m_{\text{res}}$ and Chiral Breaking $\Delta_L$}:
\vspace{-0.2em}
\[ \overline{\psi} \Delta_L \psi \simeq m_{\mathrm{res}} \overline{\psi} \psi + O(\psi, \bar \psi, A) + \mathcal{O}(a^2), \quad am_{\text{res}} = \frac{\sum_{\vec{x}} \langle J_{5q}(\vec{x}, t) P(\vec{0}, 0) \rangle}{\sum_{\vec{x}} \langle P(\vec{x}, t) P(\vec{0}, 0) \rangle} \]

It works very well at high beta case:
\vspace{0.5em}
...
\begin{columns}[c]
    ...
    \begin{column}{0.32\textwidth}
        ...
        \begin{itemize}
            \item Screening masses plotted as a function of temperature.
...
\setbeamerfont{itemize/enumerate body}{size=\fontsize{10.5pt}{12.5pt}\selectfont}
\setbeamerfont{itemize/enumerate subbody}{size=\fontsize{10.5pt}{12.5pt}\selectfont}
\vfill
